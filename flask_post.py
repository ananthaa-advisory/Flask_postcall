...
def entry():
    if request.method == "POST":
        ...
        return search(name,lname,comment)

    else:
        return render_template("post.html")

def search(name1,lname1,comment):
# THe below variable just for print to pass the HOSTNAME input to ICINGA API and process downwtime accordingly
    print(type(lname1))
    return render_template("post2.html", the_name1 = name1, the_lname1 = lname1, the_comment = comment)
